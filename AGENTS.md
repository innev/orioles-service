...
- 门户主页（应用导航、技能展示等）
- 一组独立的小应用（位于 `src/app/(apps)`，如 `2fa`、`ebook`、`etf`、`fund`、`explorer`、`nls`、`photo`、`video`、`gitmojis`、`dev-icons` 等）
- 后台管理页（`src/app/admin/dashboard`）
- 全部后端 API（`src/app/api`，另有少量 Pages Router 遗留 API 在 `src/pages/api`）

> 注：stock/stock-pool 相关功能（行情总览、股票池、A股同步分析）已抽离到独立项目 **vius（观微）**，相关代码与数据表（`Stock`/`StockData`/`Watchlist`/`AuditLog`/`AlertHistory`/`StockBasic`/`StockDaily`/`StockSignal`/`NewsFlash`）已从本仓库删除。
...
│   ├── (apps)/           # 路由分组：各小应用页面（2fa、ebook、etf、fund、explorer、nls、photo、video、gitmojis、dev-icons、cncf、qy-wechat、reports、vip、doc、github、rainbow 等）
│   ├── admin/            # 后台管理（dashboard）
│   ├── api/              # App Router API 路由（auth、apps、skills、videos、2fa、ebook、esp、etf2、explorer、icons、nls、telegram、whisper、kf、mongo_backup、docs 等）
│   ├── login/            # 登录页
│   ├── layout.tsx / page.tsx / globals.css / apps.css
├── pages/api/            # Pages Router 遗留 API（wechat）
├── model/                # 数据访问层：每个 Prisma 模型对应一个文件（User.ts 等），直接调用 @/lib/prisma
├── lib/                  # 基础设施：prisma.ts（单例客户端，唯一 PrismaClient 来源）、auth.ts（NextAuth 配置）、i18n-*.ts
├── service/              # 前端调用的 API 路径常量与封装（index.ts、aliyun.ts、wechat.ts）
├── components/           # React 组件（client/、server/、ui/、iv-ui/、layouts/ 等）
├── hooks/                # 自定义 hooks
...
- **API 响应约定**：API 路由统一返回 `{ code, data, message }` JSON 结构（**成功 code=200**，前端判断用 `code !== 200` 视为失败）；错误处理使用 `@/utils/api-response` 的 `handleApiError`。依赖 `searchParams` 的路由需声明 `export const dynamic = 'force-dynamic'`。
...
- **认证与鉴权**：
  ...
  - 页面级鉴权在 `src/middlewares/auth.ts`：`protectedRoutes`（`/dashboard`、`/2fa`、`/explorer`、`/photo`、`/video`、`/fund`、`/etf`、`/vip`、`/ebook`）未登录会 307 重定向到 `/` 并带 `auth-redirect` 参数。
