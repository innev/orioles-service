...
import SiderLinks from '@/components/client/SiderLinks';
        ...
        <div className='flex flex-col gap-4 md:gap-6'>
            <div className='content-opacity rounded-lg w-full md:w-72 flex flex-col gap-4 items-center justify-center'>
                <div className='flex flex-col items-center justify-center gap-2 pt-6 pb-2'>
                    <img className="w-28 h-28 rounded-full cursor-pointer" src={user.avatar} alt={user.nickname} onClick={() => status === 'unauthenticated' && setShowLoginModal(true)} />
                    <div className="text-xl font-medium">{user.nickname}</div>
                    <div className='text-gray-500'><span className='text-sm'>{user.bio}</span></div>
                </div>
