...
const GroupApps = ({ className = 'content-opacity md:w-80', links = [] }: { className?: string, links?: TApp[] }) => {
    ...
    return (
        <div className={`${className} rounded-lg w-full flex flex-row flex-wrap items-center justify-start gap-6 p-6`}>
            {isLoading || data.map((app: TApp) => app.visiable === true && <IconButton item={app} key={app.url} />)}
        </div>
    );
};
