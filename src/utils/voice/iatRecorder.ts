...
export default class IatRecorder {
  ...
  constructor({ business, appId }: {business: any; appId: any}) {
    this.status = 'null'
  ...
  setResultText({ resultText, resultTextTemp }: {resultText: any, resultTextTemp: any}) {
  ...
  // 连接websocket
  connectWebSocket() {
    return getWebSocketUrl('iat').then(url => {
      let iatWS
 ...
 * 使用案例如下：
  ...
  const mediaFail = () => {
    let html = <div className="mt-40 mb-22">
      <img className="mt12 w76" src={require('@epub/common/assets/microphone.png')} />
